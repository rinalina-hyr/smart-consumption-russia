...
# ============================================
#
# ВАЖНО. Категориальные данные сами по себе
# не различают две версии: «чистая экономия»
# и «избирательность». Обе предсказывают
# концентрацию роста СТМ в повседневных
# категориях, поскольку там накапливается
# максимальный эффект экономии. Мы честно
# фиксируем, что различающий сигнал —
# только мотивация, и у неё нет базовой
# линии за прошлые годы.
#
# Регрессия тренда убрана: 5 точек на грани
# значимости и допущение о Q2 делают её
# непригодной для подтверждения гипотезы.
# Динамика приводится описательно.
# ============================================
...
g2 <- ggplot(mot, aes(share_pct, factor, fill = type)) +
  ...
  scale_fill_manual(values = c(
    "Рациональный (цена, качество, состав)" = "#2E86AB",
    "Ценностный (семья, безопасность, надёжность)" = "#C0392B"
  )) +
...
# (колбасы и мясные деликатесы, кондитерка).
# Строгая их исключает. Сравниваем устойчивость
# цифры «10 из 10».
...
# ============================================
# Описательно. Без регрессии и p-value.
...
cat(sprintf("    Строгая классификация:  %d из %d — повседневные\n",
            strict_n, nrow(top10)))
cat("    Спорные: колбасы и мясные деликатесы,\n")
cat("    кондитерские изделия.\n\n")

cat("  Мотивация выбора СТМ (2026):\n")
cat("    Цена 89%, семья 81%, безопасность 78%,\n")
cat("    надёжность 76%, качество 74%.\n\n")

cat("  Динамика доли покупателей СТМ:\n")
cat("    60% (январь) → 63% (июль). Описательно.\n\n")

cat("Интерпретация:\n\n")
cat("  Категориальные данные сами по себе не различают\n")
cat("  версии «чистая экономия» и «избирательность».\n")
cat("  Обе предсказывают концентрацию роста СТМ\n")
cat("  в повседневных категориях: там накапливается\n")
cat("  максимальный эффект экономии и меньше разрыв\n")
cat("  в качестве между брендом и СТМ.\n\n")

cat("  Различающий сигнал — доля ценностных мотивов\n")
cat("  (семья, безопасность, надёжность) на уровне\n")
cat("  76–81%, сопоставимая с ценой. Чистой экономией\n")
cat("  это не объясняется.\n\n")

cat("  Но базовой линии за предыдущие годы нет:\n")
cat("  неизвестно, выросли ли ценностные мотивы\n")
cat("  за 2024–2026 или были высокими всегда.\n")
cat("  Утверждать сдвиг по одной точке нельзя.\n\n")
