...
НИУ ВШЭ, Ромир, СберИндекс, Росстат и ВЦИОМ. База — 31 таблица
...
- Ранг топ-категорий СТМ устойчив с 2022 г. (молочная
  продукция и бакалея — топ-2 в обеих точках), но доля
  СТМ внутри категорий смешанная: большинство снизилось
  на 1–4 п.п., а в товарах личной гигиены выросла почти
  вдвое (+13 п.п.). Резкий скачок именно в одной категории
  согласуется с избирательным перераспределением спроса,
  но не доказывает его без данных о мотивации за прошлые годы.
- Ценовой компонент индекса «умности» не сдвинулся (0 п.п.),
  но в окне, синхронном с индексом (окт-ноя 2024 → окт-ноя 2025),
  давление на бюджеты тоже не росло (−0,7 п.п.) — прямой тест
  здесь непроверяем. При этом остальные компоненты индекса
  выросли на +2…+5 п.п. независимо от давления: рост умности
  связан с перестройкой критериев выбора, а не с реакцией
  на ухудшение финансов.
...
| H2 | Ценовой компонент vs давление на бюджеты | Переформулирована; вывод подтвержден  | −0,7 п.п. давления, 0 п.п. цена, +3,0 п.п. остальные |
...
│ └── smart_consumption.db  31 таблица SQLite
├── r/
│ ├── H1.R  СТМ в категориях
│ ├── H2.R  ценовой компонент
│ ├── H3.R  отставание 45–54
│ ├── rebuild_database.R
...
(SQLite, 30 таблиц). Структура и источники описаны
...
- R: readxl, DBI, RSQLite, dplyr, tidyr, ggplot2, here, 
forcats, stringr.
...
![Сравнение 2022 vs 2026](visualizations/H1_stm_categories_2022_vs_2026.png)

### H2. Ценовой компонент vs давление на бюджеты
...
- **SQL** (SQLite) — аналитические запросы, 31 таблица
